...
/**
 * WPS Breadcrumbs is a class that is responsible for creating SEO-friendly
...
class WPS_Breadcrumbs {
	...
	public static function get_breadcrumb_trail( $page_id ) { 
		...
		if( ! is_home() ) {
			...
			if( is_page() ) {
				...
				if( is_author() ) {
					$str_breadcrumb .= self::get_home_link() . __( 'Archives', WPS_THEME_SLUG ) . ' ' . __( '/', WPS_THEME_SLUG ) . ' ' . self::get_author_display_name(); 
				} elseif( '' != get_query_var( 'year' ) || '' != get_query_var( 'monthnum' ) || '' != get_query_var( 'm' ) || '' != get_query_var( 'day' ) ) {
					$str_breadcrumb .= self::get_home_link() . __( 'Archives', WPS_THEME_SLUG ) . ' ' . __( '/', WPS_THEME_SLUG ) . ' ' . self::get_date_labels(); 
				} else {
					$str_breadcrumb .= self::get_home_link() . __( 'Archives', WPS_THEME_SLUG ) . ' ' . __( '/', WPS_THEME_SLUG ) . ' ' . self::get_category_links();
				} // end if
				
			} else if( is_search() ) {
			
				$str_breadcrumb .= self::get_home_link() . __( 'Search', WPS_THEME_SLUG ) . ' ' . __( '/', WPS_THEME_SLUG ) . ' ' . self::get_search_query();
	...
	private static function get_home_link() {
		...
		$home_link .= '<li class="home-breadcrumb">';
			$home_link .= '<a href="' . home_url() . '" itemprop="url"><span itemprop="title">' . __( 'Home', WPS_THEME_SLUG ) . '</span></a>';
	...
	private static function get_search_query() { 
		...
		$query = __( '[ No search query. ]', WPS_THEME_SLUG );
		if( isset( $_GET['s'] ) ) {
			$query = $_GET['s'];
		} // end if/else
